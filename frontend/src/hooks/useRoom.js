// Read-only WebSocket hook used by the OBS overlay viewer.
// For the main Room (which needs local-only mode), see useGameState.js.
    ...
    function connect() {
      ...
      ws.onmessage = (evt) => {
        try {
          ...
          if (msg.type === 'WELCOME') setYou(msg.you);
      ...
      ws.onclose = () => {
        setStatus('closed');
        if (shouldReconnectRef.current) retryTimer = setTimeout(connect, 1500);
      };
    }
